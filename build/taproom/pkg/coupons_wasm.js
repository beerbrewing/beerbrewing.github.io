...
export function process_url(url) {
    ...
    const ret = wasm.process_url(ptr0, len0);
    if (ret[2]) {
 ...
 * @returns {any}
 */
export function process_url_sanitized(url) {
    const ptr0 = passStringToWasm0(url, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.process_url_sanitized(ptr0, len0);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('coupons_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
